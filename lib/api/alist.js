	...
	list.forEach(item => {
		if(item.name!=='error') {
			...
			const vod = {
				...
				vod_id: 'no_data',
				vod_remarks: '微博@学外语i',
				vod_pic: 'https://gitee.com/iiwc/ii/raw/i/lib/pic/404'
			}
			allList.push(vod);
		}
	});
...
function getAll(otid,tid,drives,path){
	try {
		...
		print(e.message);
		let list = [{vod_name:'无数据,防无限请求',type_name: "文件夹",vod_id:'no_data',vod_remarks:'微博@学外语i',vod_pic:'https://gitee.com/iiwc/ii/raw/i/lib/pic/404',vod_actor:e.message,vod_director: tid,vod_content: otid}];
